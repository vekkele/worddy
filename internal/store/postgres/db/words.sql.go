...
type GetUserReviewsCountInRangeParams struct {
	UserID       int64
	NextReview   pgtype.Timestamptz
	NextReview_2 pgtype.Timestamptz
}

type GetUserReviewsCountInRangeRow struct {
	Count      int64
	NextReview pgtype.Timestamptz
}

func (q *Queries) GetUserReviewsCountInRange(ctx context.Context, arg GetUserReviewsCountInRangeParams) ([]GetUserReviewsCountInRangeRow, error) {
	rows, err := q.db.Query(ctx, getUserReviewsCountInRange, arg.UserID, arg.NextReview, arg.NextReview_2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUserReviewsCountInRangeRow
	for rows.Next() {
		var i GetUserReviewsCountInRangeRow
		if err := rows.Scan(&i.Count, &i.NextReview); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
